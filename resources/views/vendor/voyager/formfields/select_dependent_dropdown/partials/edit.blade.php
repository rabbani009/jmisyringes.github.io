...
?>
	...
	@php
		...
		$method = Str::camel(sprintf('%s_relationship', $row->field));		// for example:  subcategoryIdRelationship
	...
	@if ( method_exists($model, $method) )
		...
		@php 
			$model  = $model::{$method}($dataTypeContent->id);				// for example:  App\Subsubcategory::subcategoryIdRelationship($productId)
		...
		@endif
	@else
	<p class="label label-warning"><i class="voyager-warning"></i> {{ __('voyager::form.field_select_dd_relationship', ['method' => 'public static function ' . Str::camel($method) . '($id){}', 'class' => $dataType->model_name]) }}</p>
